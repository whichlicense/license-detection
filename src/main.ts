...
const unclassified = JSON.parse(
  Deno.readTextFileSync("./licenses/unclassified.json"),
);
...
if (matches.length === 0) {
  ...
  The best match is ${bestMatch[0].name} with a confidence of ${
    Math.min(100, bestMatch[0].confidence * 100)
  }%
  ...
  ${
    matches
      .map((match) =>
        `${
          Math.min(100, match.confidence * 100).toFixed(2)
        }% - ${match.name} \t Common blocks: ${match.commonBlocks}/${match.totalBlocks}`
      )
      .join("\n")
  }
  `);
