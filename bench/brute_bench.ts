...
const LICENSE_FILES = Array.from(Deno.readDirSync("./licenses/RAW")).map((
  file,
) => `./licenses/RAW/${file.name}`);

function randomArrItem<T>(arr: T[]): T {
  return arr[Math.floor(Math.random() * arr.length)];
}

const PICKED_LICENSES = Array.from(
  { length: ITERATIONS },
...
Deno.bench(
  ...
  async () => {
    ...
    for (const license of PICKED_LICENSES) {
      // send and forget..
      promisePool.push(
        DETECTION_SCHEDULER.detectLicense(cloneByteArray(license)),
      );
    }
