...
import { computeAllLicenseHashes, DEFAULT_BLOCK_SIZE, DEFAULT_FUZZY_HASH_LENGTH } from "../src/scripts/computeLicenses.ts";
...
Deno.bench(`Single license [${DEFAULT_BLOCK_SIZE}, ${DEFAULT_FUZZY_HASH_LENGTH}] (BASELINE)`, {group: 'sld', baseline: true}, () => {
...
for(let blockSize = MIN_BLOCK_SIZE; blockSize <= MAX_BLOCK_SIZE; blockSize++){
    ...
    const TMEP_FILE_ENTRY = Deno.makeTempFileSync({prefix: `license_${blockSize}_${fuzzyHashLength}_`});

    Deno.writeTextFileSync(TMEP_FILE_ENTRY, JSON.stringify(ADJUSTED_LICENSE_DB));
