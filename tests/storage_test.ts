...
import { assert, assertEquals, assertExists, fail } from "https://deno.land/std@0.177.0/testing/asserts.ts";
...
Deno.test("Empty storage tests", {},
  async (t) => {
    const EMPTY_TEST_FILE = Deno.makeTempFileSync({ prefix: "license_storage_test_" });
    ...
    await t.step("Adding a license increments entry", () => {
        EMPTY_STORAGE.addLicense({
            name: "APACHE-2.0",
            hash: computeLicenseHash(new TextEncoder().encode(TEST_LICENSE_1), 10 ,7).hash,
            blockSize: 10,
            fuzzyHashLength: 7
        })
...
Deno.test("Populated storage tests", {},
 async (t) => {
    ...
    STORAGE.addLicense({
        name: "TEST_2",
        hash: computeLicenseHash(new TextEncoder().encode(TEST_LICENSE_2), 10 ,7).hash,
        blockSize: 10,
        fuzzyHashLength: 7
    })
    ...
    STORAGE.addLicense({
        name: "TEST_3",
        hash: computeLicenseHash(new TextEncoder().encode(TEST_LICENSE_3), 10 ,7).hash,
        blockSize: 10,
        fuzzyHashLength: 7
    })
    ...
    await t.step("parseEntry works", () => {
        ...
        for(const entry of STORAGE.entries()) {
            ...
            assertEquals(parsed!.hash,computeLicenseHash(new TextEncoder().encode( TEST_LICENSE_MAP[`TEST_LICENSE_${i+1}`] ), 10 ,7).hash)
            i++;
        }
    });
    await t.step("Batched entries works", () => {
        ...
        for(const entry of STORAGE.entriesBatched(2)) {
            if(i == 0) {
                ...
                assertEquals(parsed1!.hash,computeLicenseHash(new TextEncoder().encode( TEST_LICENSE_MAP[`TEST_LICENSE_1`] ), 10 ,7).hash)
                assertEquals(compareHashes(parsed1!.hash, computeLicenseHash(new TextEncoder().encode( TEST_LICENSE_MAP[`TEST_LICENSE_1`] ), 10 ,7).hash).confidence, 1)
                ...
                assertEquals(parsed2!.hash,computeLicenseHash(new TextEncoder().encode( TEST_LICENSE_MAP[`TEST_LICENSE_2`] ), 10 ,7).hash)
                assertEquals(compareHashes(parsed2!.hash, computeLicenseHash(new TextEncoder().encode( TEST_LICENSE_MAP[`TEST_LICENSE_2`] ), 10 ,7).hash).confidence, 1)
                ...
                assertEquals(parsed3!.hash,computeLicenseHash(new TextEncoder().encode( TEST_LICENSE_MAP[`TEST_LICENSE_3`] ), 10 ,7).hash)
                assertEquals(compareHashes(parsed3!.hash, computeLicenseHash(new TextEncoder().encode( TEST_LICENSE_MAP[`TEST_LICENSE_3`] ), 10 ,7).hash).confidence, 1)
